...
export class User extends BaseModel<UserSchema, NexusUser, UserPK> implements NexusUser {
  ...
  public static toSchema(user: NexusUser, overrides: Partial<UserSchema> = {}): UserSchema {
    const now = Date.now();
    return {
      id: user.details.id,
