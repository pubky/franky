...
export async function createFollowRelationship(followerId: UserPK, followedId: UserPK): Promise<void> {
  await db.transaction('rw', db.users, async () => {
    // Update follower
    await db.users.where('id').equals(followerId).modify(user => {
    ...
    await db.users.where('id').equals(followedId).modify(user => {
 ...
 */
export async function createTagRelationship(
  taggerId: UserPK,
  taggedId: UserPK,
  label: string
): Promise<void> {
  await db.users.where('id').equals(taggedId).modify(user => {
