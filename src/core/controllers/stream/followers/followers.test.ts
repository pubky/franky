...
import { StreamFollowersController } from './followers';

describe('StreamFollowersController', () => {
  ...
  describe('getOrFetchStreamSlice', () => {
    it('should return users when no cache misses', async () => {
      ...
      const getOrFetchStreamSliceSpy = vi
        .spyOn(Core.FollowersStreamApplication, 'getOrFetchStreamSlice')
        .mockResolvedValue({
          nextPageIds,
          cacheMissUserIds: [],
          skip,
      ...
      const fetchMissingUsersSpy = vi.spyOn(Core.FollowersStreamApplication, 'fetchMissingUsersFromNexus');

      const result = await StreamFollowersController.getOrFetchStreamSlice({
        streamId,
        user_id: userId,
        limit: 10,
      });
    ...
    it('should fetch missing users when cacheMissUserIds exist', async () => {
      ...
      const fetchMissingUsersSpy = vi
        .spyOn(Core.FollowersStreamApplication, 'fetchMissingUsersFromNexus')
        .mockResolvedValue();
    ...
    it('should use default limit when not provided', async () => {
      ...
      await StreamFollowersController.getOrFetchStreamSlice({
        streamId,
        user_id: userId,
      });
    ...
    it('should not fetch missing users when cacheMissUserIds is empty array', async () => {
      ...
      vi.spyOn(Core.FollowersStreamApplication, 'getOrFetchStreamSlice').mockResolvedValue({
        nextPageIds,
        cacheMissUserIds: [],
        skip,
      });
