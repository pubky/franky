...
export class PostStreamApplication {
  ...
  static async getOrFetchStreamSlice({ streamId, limit = Config.NEXUS_POSTS_PER_PAGE, post_id, timestamp }: Core.TReadStreamPostsParams): Promise<Core.TPostPaginationResponse> {
  ...
  static async fetchMissingPostsFromNexus(postIds: string[]) {
    let { url, body } = Core.postStreamApi.postsByIds({ post_ids: postIds/*, viewer_id */ });
    ...
    let { url: userUrl, body: userBody } = Core.userStreamApi.usersByIds({ user_ids: cacheMissUserIds/*, viewer_id */ });
  ...
  private static async fetchStreamFromNexus({streamId, limit, post_id/*, timestamp*/}: Core.TReadStreamPostsParams): Promise<Core.TPostPaginationResponse> {
    ...
    const compositePostIds = nexusPosts.map((post) => Core.buildPostCompositeId({ pubky: post.details.author, postId: post.details.id }));
  ...
  private static async getNotPersistedPostsInCache(postIds: string[]): Promise<string[]> {
    let existingPostIds = await Core.PostDetailsModel.findByIdsPreserveOrder(postIds);
  ...
  private static async getNotPersistedUsersInCache(userIds: Core.Pubky[]): Promise<Core.Pubky[]> {
    let existingUserIds = await Core.UserDetailsModel.findByIdsPreserveOrder(userIds);
    return userIds.filter((_userId, index) => existingUserIds[index] === undefined);
  }

  private static getStreamFromCache(postId: string | undefined, limit: number, cachedStream: { stream: string[] }): string[] | null {
    ...
    if (!postId && cachedStream.stream.length >= limit) {
      return cachedStream.stream.slice(0, limit - 1);
    }
    const postIndex = postId ? cachedStream.stream.indexOf(postId) : -1;
    if (postIndex !== -1 && postIndex + limit <= cachedStream.stream.length) {
      return cachedStream.stream.slice(postIndex + 1, postIndex + limit);
    }
    return null;
  }
}
