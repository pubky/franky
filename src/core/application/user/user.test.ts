...
describe('UserApplication.search', () => {
  ...
  it('should delegate to NexusUserService.search with correct parameters', async () => {
    const searchSpy = vi.spyOn(Core.NexusUserService, 'search').mockResolvedValue(mockUserIds);

    const result = await UserApplication.search({ prefix: 'test', limit: 10 }, 'name');

    expect(result).toEqual(mockUserIds);
    expect(searchSpy).toHaveBeenCalledWith({ prefix: 'test', limit: 10 }, 'name');
  });

  it('should pass id search type to service', async () => {
    const searchSpy = vi.spyOn(Core.NexusUserService, 'search').mockResolvedValue(mockUserIds);

    await UserApplication.search({ prefix: 'qr3x' }, 'id');

    expect(searchSpy).toHaveBeenCalledWith({ prefix: 'qr3x' }, 'id');
  });

  it('should return empty array when service returns empty', async () => {
    vi.spyOn(Core.NexusUserService, 'search').mockResolvedValue([]);

    const result = await UserApplication.search({ prefix: 'nonexistent' }, 'name');

    expect(result).toEqual([]);
  });

  it('should propagate errors from service layer', async () => {
    vi.spyOn(Core.NexusUserService, 'search').mockRejectedValue(new Error('Network error'));

    await expect(UserApplication.search({ prefix: 'test' }, 'name')).rejects.toThrow('Network error');
  });
});
