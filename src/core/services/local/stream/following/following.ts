...
/**
 * Local Stream Following Service
 *
 * Simple service to manage following stream IDs in IndexDB.
 * Only stores arrays of user IDs (Pubky), no user data.
 */
export class LocalStreamFollowingService {
  private constructor() {}

  /**
   * Save or update a stream of following IDs
   */
  static async upsert({ streamId, stream }: Core.TFollowingStreamUpsertParams): Promise<void> {
    await Core.UserStreamModel.upsert(streamId, stream);
  }

  /**
   * Get a stream of following IDs by stream ID
   */
  ...
  /**
   * Delete a following stream from cache
   */
  ...
  /**
   * Append new user IDs to an existing following stream
   ...
   */
  static async persistNewStreamChunk({ stream, streamId }: Core.TFollowingStreamUpsertParams): Promise<void> {
    const userStream = await Core.UserStreamModel.findById(streamId);
    if (!userStream) {
      throw new Error(`Following stream not found: ${streamId}`);
    }
