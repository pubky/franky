...
describe('LocalStreamFollowersService', () => {
  ...
  const NON_EXISTENT_STREAM_ID = Core.UserStreamTypes.TODAY_FOLLOWERS_ALL;
  ...
  const persistAndVerifyUser = async (userId: Core.Pubky, overrides?: Partial<Core.NexusUser>) => {
    ...
    const result = await Core.LocalStreamFollowersService.persistUsers([mockUser]);
  ...
  describe('upsert', () => {
    ...
    it('should update an existing stream with new user IDs', async () => {
      ...
      await Core.LocalStreamFollowersService.upsert({ streamId, stream: updatedIds });

      await verifyStream(updatedIds);
    });

    it('should handle empty array', async () => {
      await Core.LocalStreamFollowersService.upsert({ streamId, stream: [] });
  ...
  describe('findById', () => {
    it('should return stream when it exists', async () => {
      ...
      const result = await Core.LocalStreamFollowersService.findById(streamId);
    ...
    it('should return null when stream does not exist', async () => {
      const result = await Core.LocalStreamFollowersService.findById(NON_EXISTENT_STREAM_ID);
  ...
  describe('deleteById', () => {
    it('should delete an existing stream', async () => {
      const userIds: Core.Pubky[] = ['user1' as Core.Pubky, 'user2' as Core.Pubky];
      await createStream(userIds);
      await Core.LocalStreamFollowersService.deleteById(streamId);
      await verifyStreamDoesNotExist();
    });

    it('should not throw error when deleting non-existent stream', async () => {
      await expect(Core.LocalStreamFollowersService.deleteById(NON_EXISTENT_STREAM_ID)).resolves.not.toThrow();
    });
  });
  ...
  describe('persistUsers', () => {
    it('should persist users and return user IDs', async () => {
      ...
      const result = await Core.LocalStreamFollowersService.persistUsers(mockUsers);
    ...
    it('should handle empty array', async () => {
      const result = await Core.LocalStreamFollowersService.persistUsers([]);
  ...
  describe('persistNewStreamChunk', () => {
    it('should append new stream chunk to existing stream', async () => {
      ...
      await Core.LocalStreamFollowersService.persistNewStreamChunk({
        streamId,
        stream: newChunk,
      });
    ...
    it('should throw error when stream does not exist', async () => {
      await expect(
        Core.LocalStreamFollowersService.persistNewStreamChunk({
          streamId: NON_EXISTENT_STREAM_ID,
          stream: ['user-1' as Core.Pubky],
        }),
      ).rejects.toThrow(`Followers stream not found: ${NON_EXISTENT_STREAM_ID}`);
    });
