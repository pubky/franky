  ...
  return (
    <>
      {menuItems.map((item) => {
        ...
        const textColor = item.variant === 'destructive' ? 'text-destructive' : 'text-muted-foreground';
        const iconColor = item.variant === 'destructive' ? 'text-destructive' : 'text-muted-foreground';
        ...
        return variant === 'sheet' ? (
          ...
          >
            <Atoms.Container overrideDefaults className="flex items-center gap-2">
              <Icon className={Libs.cn('size-4', iconColor)} />
              <Atoms.Typography as="span" overrideDefaults className={Libs.cn('text-base font-medium', textColor)}>
                {item.label}
              </Atoms.Typography>
            </Atoms.Container>
          </Atoms.Button>
          ...
          >
            <Atoms.Container overrideDefaults className="flex items-center gap-2 p-0">
              <Icon className={Libs.cn('size-4 transition-colors', iconColor, 'group-hover:text-foreground')} />
              <Atoms.Typography
                as="span"
                overrideDefaults
                className={Libs.cn('text-base font-medium transition-colors', textColor, 'group-hover:text-foreground')}
              >
                {item.label}
              </Atoms.Typography>
            </Atoms.Container>
