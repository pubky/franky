  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      >
        {children}
        {showCloseButton && (
          <DialogClose className="absolute right-8 top-8 w-8 h-8 inline-flex items-center justify-center whitespace-nowrap text-sm transition-all duration-300 ease-in-out disabled:pointer-events-none disabled:opacity-50 bg-secondary text-secondary-foreground hover:bg-secondary/80 rounded-full cursor-pointer ">
...
function DialogHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={Libs.cn('flex flex-col gap-2 text-center sm:text-left', className)}
      {...props}
    />
  );
}
